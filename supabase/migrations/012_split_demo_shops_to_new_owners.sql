...
-- at signup." Two dev accounts had more than one shop under this
-- project's history:
--   - sabstore.test.owner@gmail.com ("Suresh Sharma") had 4 shops —
--     this splits 3 off, leaving that owner with just "SabStore."
--   - maqbooln2022@gmail.com had 2 shops — this splits "City
--     Supermarket" off, leaving that owner with just "Sabstore."
--
-- Run this ONLY after all 4 new owners below have signed up through the
...
--   City Supermarket -> maqbooln2022+citysupermarket@gmail.com
--
...
declare
  ...
  v_new_name text;
begin
  select id into v_old_owner from auth.users where email = 'sabstore.test.owner@gmail.com';
...
begin
  ...
  select id into v_new_owner from auth.users where email = 'maqbooln2022+citysupermarket@gmail.com';
  if v_new_owner is null then
    raise exception 'maqbooln2022+citysupermarket@gmail.com has not signed up yet — create that account in the app first';
  end if;

  select id into v_shop_id from shops where name = 'City Supermarket' and owner_id = v_old_owner;
  if v_shop_id is null then
    raise notice 'City Supermarket not found under the original owner — already migrated, skipping';
  else
    select coalesce(raw_user_meta_data->>'full_name', 'Owner') into v_new_name from auth.users where id = v_new_owner;
    update shops set owner_id = v_new_owner where id = v_shop_id;
    delete from shop_members where shop_id = v_shop_id and user_id = v_old_owner;
    insert into shop_members (shop_id, user_id, role, name, permissions)
      values (v_shop_id, v_new_owner, 'owner', v_new_name,
        '{"dashboard":true,"inventory":true,"billing":true,"history":true,"credit":true,"dayclose":true,"expenses":true,"cashbook":true,"suppliers":true}'::jsonb)
      on conflict (shop_id, user_id) do update set role = 'owner', permissions = excluded.permissions;
    raise notice 'City Supermarket reassigned to %', v_new_owner;
  end if;
end $$;
